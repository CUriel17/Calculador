...
function change_subnet_number(){
    ...
    c=c.replace(/^\s+|\s+$/g,"");
    if(isNaN(c)){
        alert("Ingrese un número entero entre 2 y 999")
    }
    else{
        if((c%1)!==0){
            alert("Ingrese un número entero, sin decimales ni comas, por favor")
        }
        else{
            if((c>999)||(c<2)){
                alert("Ingrese un número entero positivo entre 2 y 999")
            }
            else{paragraph="<span class='column'>Nombre de Subred:</span> <span class='column'>Number de Hosts:</span><br>";
            for(var b=1;b<=c;b++){
...
function vlsm(){
    ...
    if(!validate(h)){
        document.getElementById("not_valid_ip").innerHTML="<b>Esto no parece una red válida</b>";
        return
    }
    ...
    var s="<p>IP Network "+c[0]+"."+c[1]+"."+c[2]+"."+c[3]+"/"+n+" Tiene "+g+" hosts.<br>Requieres "+a+" hosts.</p>";
    var t="<table border='1'><tr><td>ID SUBRED</td><td>Hosts Requeridos</td><td>Hosts Disponibles</td><td>Hosts no Usados</td><td>IP Network</td><td>Prefijo</td><td>Mas. de Subred</td><td>Rango IP</td><td>Broadcast</td><td>Wildcard</td></tr>";
    var b=c;
    var k=0;
    for(var r=0;r<d.length;r++){
        ...
        k+=(find_hosts(j)+2);
        t+="<tr><td>"+d[r][1]+"</td>";
        t+="<td>"+d[r][0]+"</td>";t+="<td>"+find_hosts(j)+"</td>";t+="<td>"+(find_hosts(j)-d[r][0])+"</td>";
    ...
    t+="</table>";
    if(k>g+2){
        s+="<span style='background-color:yellow;'>Parece que esas subredes no encajarán en esa red, pero aquí hay algo más que puede funcionarte :v</span><br>"
    }
    t=s+t;
...
function find_mask(c){
    ...
    if(c<8){
        ...
        if(c<16){
            a[0]=255;a[1]=256-Math.pow(2,(32-(c+16)))
        }
        else{
            if(c<24){
                a[0]=255;
...
function ordered_hosts(a){
    ...
    for(var f=1;f<=a;f++){
        var e="name"+f;
        var d="hosts"+f;
        e=document.getElementById(e).value;d=document.getElementById(d).value;
        if(d>=1){
            c[b]=[d,e];
            b++
        }
    }
    c.sort(function(h,g){return g[0]-h[0]});
    return c
}
function sum_hosts(a){
    ...
    for(var c=1;c<=a;c++){
        var b="hosts"+c;
        b=parseInt(document.getElementById(b).value);if(b>=1){
            d+=b
        }
    }
    return d
